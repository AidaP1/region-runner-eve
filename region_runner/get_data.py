...
def get_data():
    ...
    if request.method == 'POST':
        ...
        for region in regions_to_pull:
            ...
            with current_app.open_resource('./get_esi/region-summary.sql') as f:
                try:
                    display_orders = db.executescript(f.read().decode('utf8')).fetchall()
                    flash(display_orders)
                except db.Error as er:
                    error = er
                ...
                for o in display_orders:
                    o1 = {}
                    o1['station_id'] = o['station_id']
                    o1['count'] = o['count']
                    o_list.append(o1)
                
                if error is None:
                        return render_template('/data/get-data.html', regions=regions, o_list=o_list)
